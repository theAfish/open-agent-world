...
/** The PDF and its extractions are Paper files; import is a resource action, not a document write. */
async function importInto(id:string, file:File) {
  return api(`nodes/${id}/resource/import`,{arguments:{filename:file.name,pdf:await encoded(file)}});
}
type Doc = PaperDoc;
...
function PaperMagazine(props:PluginViewProps) {
  ...
  const [focus,setFocus]=useState<Loc&{key:number}>();
  ...
  useEffect(()=>{let active=true;void api(`library/papers/${card.id}/preview?details=true`).then(d=>{if(active){setSummary(d);setError("");}}).catch(e=>{if(active)setError(String(e));});return()=>{active=false;};},[card.id,card.updated_at,open,refresh]);
  ...
  return <div ref={root} className="library-magazine nodrag nopan nowheel">
    ...
    {summary?.value.sha256&&<ExtractionPanel paperId={card.id} name={card.name} active={summary.value.extraction?.id??null} onChanged={()=>setRefresh(n=>n+1)}
      onLocate={loc=>{setFocus({...loc,key:Date.now()});setOpen(true);}}/>}
    {open&&<ActiveReader key={attempt} {...props} focus={focus} onRetry={()=>setAttempt(n=>n+1)} onClose={()=>{setOpen(false);setFocus(undefined);}}/>}
  </div>;
}
export function ActiveReader({card,onClose,onRetry,focus}:PluginViewProps & {onClose:()=>void;onRetry:()=>void;focus?:Loc&{key:number}}) {
  useLocale();
  ...
  useEffect(()=>{if(error)fail(error);else if(doc&&!doc.value.pdfUrl)fail(t("此节点尚未导入 PDF。"));},[doc,error,fail]);
  ...
  function updateDocument(args:Record<string,unknown>){
    const next=writes.current.catch(()=>{}).then(async()=>{const current=latest.current;if(!current)return;
      const updated=withSource(await api(`nodes/${card.id}/actions/annotate`,{arguments:args,expected_revision:current.revision}),current.value);latest.current=updated;setDoc(updated);
  ...
  return createPortal(<dialog ref={readerRef} aria-label={card.name} data-entrance-phase={phase} className="library-reader nodrag nopan nowheel" onCancel={e=>{e.preventDefault();resizeReader(false);}}>
    ...
    {phase==="failed"&&<div className="library-reader-failure" role="alert"><p>{failure}</p>
      {doc&&!doc.value.pdfUrl&&<label>{t("导入 PDF")}<input type="file" accept=".pdf" onChange={async event=>{const file=event.target.files?.[0];if(!file)return;try{await importInto(card.id,file);forgetPaper(card.id);onRetry();}catch(error){fail(String(error));}}}/></label>}
    ...
    <div className="library-reader-toolbar">
      ...
      {doc?.value.pdfUrl&&<button type="button" aria-label={t("Library 设置")} title={t("Library 设置")} aria-expanded={settingsOpen} onClick={()=>setSettingsOpen(open=>!open)}>⚙</button>}
    ...
    {error&&<p role="alert">{error}</p>}
    {doc?.value.pdfUrl&&mountReader&&phase!=="failed"&&<Suspense fallback={null}><PdfReading onReady={markReady} onPreparing={invalidate} onLoadError={fail} value={doc.value} focus={focus} save={updateDocument} fullscreen={fullscreen} settingsOpen={settingsOpen} setSettingsOpen={setSettingsOpen}/></Suspense>}
    </div>
...
function Region({card,host}:PluginViewProps) {
  ...
  async function upload(files:FileList|null) {
    ...
    try {let i=0;for(const file of Array.from(files)) {if(!file.name.toLowerCase().endsWith(".pdf"))continue;
      ...
      try{await importInto(node.id,file);}
