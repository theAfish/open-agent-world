...
async function request(path:string, init?:RequestInit) {
  ...
  if(!response.ok)throw new Error(typeof data.detail==="string"?data.detail:t("请求失败"));
  return data;
}
  ...
  return <div ref={reading} className={`library-reading ${annotating?"is-annotating":""}`}>
    ...
    <div className="library-annotation-tools">
      ...
      <input hidden ref={imageInput} type="file" accept="image/png,image/jpeg" onChange={async e=>{const file=e.target.files?.[0];e.target.value="";if(!file)return;try{
        if(file.size>4*1024*1024)throw new Error(t("图片请小于 4 MiB"));
    ...
    {error&&<p role="alert">{error}</p>}
      {settingsOpen&&<section className="library-settings-popover" role="dialog" aria-label={t("Library 设置")}>
        ...
        <p>{t("对所有文献生效。API 地址和密钥统一在 OAW 总设置中管理。")}</p>
        <label>{t("翻译服务")}<select value={provider} onChange={e=>useLibrarySettings.setState({provider:e.target.value})}><option value="openai">{t("OAW 模型连接")}</option><option value="deepl">{t("DeepL Free")}</option></select></label>
        {provider==="openai"&&<label>{t("翻译模型")}<select value={model} onChange={e=>useLibrarySettings.setState({model:e.target.value})}>{models.map(item=><option key={item.value} value={item.value}>{item.label}</option>)}</select></label>}
    ...
    <button type="button" className="library-sidebar-toggle" aria-label={sidebarOpen?t("收起目录与批注"):t("展开目录与批注")} title={sidebarOpen?t("收起目录与批注"):t("展开目录与批注")} aria-expanded={sidebarOpen} onClick={()=>setSidebarOpen(open=>!open)}>{sidebarOpen?"›":"‹"}</button>
    <aside className="library-reading-sidebar" aria-label={t("目录与批注")} aria-hidden={!sidebarOpen}>
    ...
    {selection&&<div ref={popup} role="dialog" aria-label={t("划词批注与翻译")} aria-modal="false" className={`library-selection-popup nodrag nopan nowheel ${selection.annotation?"is-excerpt-bar":""}`} style={{left:popupPosition?.left??0,top:popupPosition?.top??0,visibility:popupPosition?.visible?"visible":"hidden"}}>
      {selection.annotation?<>
        ...
        {["#f4d144","#71c98b","#6aaef5","#f08080"].map((color,i)=><button key={color} aria-label={[t("黄色"),t("绿色"),t("蓝色"),t("红色")][i]} className="library-color-dot" style={{background:color}} onClick={()=>changeExcerpt({color})}/>)}
        <button onClick={()=>{setEditing(selection.annotation);dismissSelection();}}>{t("编辑")}</button><button aria-label={t("关闭摘录浮窗")} onClick={dismissSelection}>×</button>
      </>:<>
      <header><strong>{t("Text annotation · Page {page}", { page })}</strong><button aria-label={t("关闭划词浮窗")} disabled={saving} onClick={dismissSelection}>×</button></header>
      <blockquote>{selection.text}</blockquote>
      <label>{t("批注")}<textarea placeholder={t("写下想法，留空则仅保存高亮")} value={comment} onChange={e=>setComment(e.target.value)}/></label>
      <div className="library-popup-actions"><button disabled={busy||saving} onClick={()=>void add()}>{saving?t("保存中…"):t("保存高亮与批注")}</button><button disabled={busy||saving} onClick={()=>void translate()}>{busy?t("翻译中…"):t("翻译选段")}</button></div>
      {translation&&<><p className="library-translation">{translation}</p><button onClick={()=>void navigator.clipboard.writeText(translation).catch(e=>setError(String(e)))}>{t("复制译文")}</button><small>{t("点击保存，将译文和批注一起归档。")}</small></>}
      {error&&<p role="alert">{error}</p>}
    ...
    {editing&&<div className="library-edit-backdrop"><section className="library-excerpt-editor" role="dialog" aria-label={t("编辑摘录")}>
      ...
      <label>{t("标题")}<input value={editing.title??""} onChange={e=>setEditing({...editing,title:e.target.value})}/></label>
      {editing.image?<img src={editing.image}/>:<blockquote>{editing.text}</blockquote>}
      <label>{t("批注")}<textarea value={editing.comment} onChange={e=>setEditing({...editing,comment:e.target.value})}/></label>
      <button disabled={saving} onClick={async()=>{setSaving(true);try{await storeAnnotation(editing);setEditing(undefined);}catch{}finally{setSaving(false);}}}>{saving?t("保存中…"):t("保存")}</button>
    </section></div>}
  </div>;
}
