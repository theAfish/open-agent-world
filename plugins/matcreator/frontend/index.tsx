...
function GraphWorkspace({ card, host }: PluginViewProps) {
  ...
  const load = useCallback(async (ids: string[] = [], append = false, offset = 0) => {
    ...
    const next = data.nodes.map((entry) => {
      ...
      return { id: entry.id, position: positions.get(entry.id)!, data: { label: entry.title }, className: `kdg-node kdg-${entry.type}`, style: { width: 110, height: 110 }, connectable: false };
    });
  ...
  const select = async (id: string) => {
    ...
    setDetail(response.value as Detail); setRevision(response.revision); setInspector(true);
  };
  const mutate = async (operation: string, args: Record<string, unknown>) => {
    await host.documentAction(operation, args, revision); setDetail(null); await load();
  };
  ...
  return <section ref={workspaceRef} className="kdg-workspace nodrag nowheel" aria-label="Know-Do Graph workspace" onPointerDown={e => e.stopPropagation()}>
    ...
    <div className="kdg-regions">
      {navigation && <aside className="kdg-navigator">
        <label>Mode<select aria-label="Workspace mode" value={mode} onChange={e => { setMode(e.target.value); setDetail(null); }}><option value="browse">Browse</option><option value="edit">Edit</option><option value="review">Review memories</option></select></label>
        ...
        {mode === "edit" && <button onClick={newEntry}>New entry</button>}
      ...
      <main className="kdg-canvas"><ReactFlow id={`oaw-knowledge-map-${card.id}`} nodes={visibleNodes} edges={visibleEdges} edgeTypes={mapEdgeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange}
        ...
        nodesConnectable={false} deleteKeyCode={null} selectionOnDrag onlyRenderVisibleElements minZoom={0.1} maxZoom={2} fitView><Background /><Controls showInteractive={false} /></ReactFlow></main>
      {inspector && <aside className="kdg-inspector"><button className="kdg-inspector-close" aria-label="Close inspector" onClick={() => setInspector(false)}>Close</button>{inspectEntry ? <>
        <h3>{inspectEntry.title}</h3><p>{inspectEntry.type} · {inspectEntry.verification} · trust {inspectEntry.trust}</p>
        <button onClick={() => void run(() => load([inspectEntry.id], true))}>Expand neighborhood</button>
        <p>{inspectEntry.summary}</p>
        {mode === "browse" && <div className="kdg-content">{inspectEntry.content}</div>}
        {mode === "edit" && <><label>Title<input value={inspectEntry.title} onChange={e => setDetail({ ...detail, entry: { ...inspectEntry, title: e.target.value } })} /></label>
          ...
          <label>Summary<textarea value={inspectEntry.summary} onChange={e => setDetail({ ...detail, entry: { ...inspectEntry, summary: e.target.value } })} /></label>
          ...
          <label>Content<textarea value={inspectEntry.content} onChange={e => setDetail({ ...detail, entry: { ...inspectEntry, content: e.target.value } })} /></label>
          <button disabled={busy} onClick={() => void run(() => mutate("edit", { entry_id: inspectEntry.owner === "publisher" ? undefined : inspectEntry.id || undefined, title: inspectEntry.title, type: inspectEntry.type, content: inspectEntry.content, summary: inspectEntry.summary, tags: inspectEntry.tags, aliases: inspectEntry.aliases, trust: inspectEntry.trust, verification: inspectEntry.verification }))}>{inspectEntry.owner === "publisher" ? "Save user-owned copy" : "Save changes"}</button>
          {inspectEntry.owner === "user" && inspectEntry.id && <button onClick={() => { if (window.confirm(`Delete “${inspectEntry.title}” and its relationships?`)) void run(() => mutate("delete_entry", { entry_id: inspectEntry.id })); }}>Delete entry</button>}
          <label>Connect to<select value={connectTarget} onChange={e => setConnectTarget(e.target.value)}><option value="">Choose entry…</option>{nodes.filter(n => n.id !== inspectEntry.id).map(n => <option key={n.id} value={n.id}>{String(n.data.label)}</option>)}</select></label>
          ...
          <button disabled={!connectTarget || busy} onClick={() => void run(() => mutate("connect", { source: inspectEntry.id, target: connectTarget, relation: connectRelation }))}>Commit relationship</button></>}
        {mode === "review" && <><label>Reviewed knowledge<textarea value={inspectEntry.content} onChange={e => setDetail({ ...detail, entry: { ...inspectEntry, content: e.target.value } })} /></label><label>Evidence<input value={evidence} onChange={e => setEvidence(e.target.value)} /></label><button disabled={!evidence || busy} onClick={() => void run(() => mutate("distill", { memory_ids: [inspectEntry.id], title: inspectEntry.title, content: inspectEntry.content, evidence }))}>Distill to Heuristic</button></>}
        <h4>Resources</h4>{detail.resources.map(resource => <details key={resource.skill_node_id}><summary>{resource.path}</summary><p>Skill resource: {resource.skill_node_id}</p>{resource.files.map(path => <button key={path} onClick={() => void run(async () => { const response = await host.documentAction("inspect", { entry_id: inspectEntry.id, resource_path: path }); setNotice(JSON.stringify(response.value)); })}>{path}</button>)}</details>)}
        <h4>Relationships</h4>{detail.relationships.map(edge => <div key={edge.id}><button onClick={() => void run(() => select(edge.source === inspectEntry.id ? edge.target : edge.source))}>{edge.relation}</button>{mode === "edit" && <button onClick={() => { if (window.confirm(`Delete ${edge.relation} relationship?`)) void run(() => mutate("delete_relationship", { edge_id: edge.id })); }}>Delete relationship</button>}</div>)}
