...
test("Sandbox local settings reuse the environment editor and a live default profile", async ({ page, request }) => {
  ...
  try {
    ...
    await expect(card.getByText("Settings saved", { exact: true })).toBeVisible();
    await card.getByText("Environment variables", { exact: true }).click();
    await card.getByRole("combobox", { name: "Default profile", exact: true }).selectOption(profile.id);
    ...
    await expect(workspace.getByRole("combobox", { name: "Default profile", exact: true })).toHaveValue(profile.id);
    await expect(workspace.getByLabel("Environment variable 1 name")).toHaveValue("REGION");
    await expect(workspace.getByLabel("Environment variable 1 value")).toHaveValue("local");
    await expect(workspace.getByLabel("Command timeout (seconds)")).toHaveValue("90");
