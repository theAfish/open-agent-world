...
test("manage multiple accounts, persist models, and keep settings navigation on the left", async ({ page }) => {
  ...
  await dialog.getByLabel("Default for new agents").selectOption({ label: "Work account / Work assistant" });
  ...
  await expect(dialog.getByLabel("Default for new agents")).toContainText("Work account / Work assistant");
