...
it("passes drop membership and removes only the new node after import failure",async()=>{
  ...
  mockUpload(400);
  const fetch=vi.fn().mockResolvedValueOnce({ok:true,json:async()=>({id:"created"})}).mockResolvedValueOnce({ok:true});
  ...
  expect(fetch.mock.calls[1]).toEqual(["/api/nodes/created",{method:"DELETE"}]);
});
...
it("synchronizes each imported node without refresh and reports real upload progress",async()=>{
  ...
  vi.stubGlobal("fetch",vi.fn().mockResolvedValueOnce({ok:true,json:async()=>node}).mockResolvedValueOnce({ok:true,json:async()=>node}));
...
it("keeps successful imports when the summary read fails",async()=>{
  vi.stubGlobal("FileReader",class {result="data:application/pdf;base64,QQ==";onload?:()=>void;readAsDataURL(){this.onload?.();}});
  mockUpload();
  const fetch=vi.fn().mockResolvedValueOnce({ok:true,json:async()=>({id:"saved",type:"library.paper"})}).mockRejectedValueOnce(new Error("offline"));
  vi.stubGlobal("fetch",fetch);
