...
export async function importPdf(file:File, position:{x:number;y:number}, parentId?:string,
  ...
  if(file.size>25*1024*1024)throw new Error(`${file.name}: 每个 PDF 最大 25 MiB`);
  ...
  try {
    ...
    await new Promise<void>((resolve,reject)=>{
      ...
      xhr.onerror = () => reject(new Error("PDF 上传连接中断"));
  ...
  catch(error){const cleanup=await fetch(`/api/nodes/${node.id}`,{method:"DELETE"});if(!cleanup.ok)throw new Error(`${String(error)}；未能清理新建空节点 ${node.id}`);throw error;}
