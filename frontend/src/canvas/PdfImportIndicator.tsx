...
export function PdfImportIndicator({status,progress,onDismiss}: {
  ...
  useLocale();
  const label=progress && {reading:t("读取文件"),uploading:t("上传进度"),processing:t("上传完成 · 正在解析 PDF"),complete:t("导入完成")}[progress.stage];
  return <div className="nodrag nopan" style={{position:"absolute",top:16,left:"50%",transform:"translateX(-50%)",zIndex:1000,padding:"10px 14px",borderRadius:12,background:"#302e28",border:"1px solid #555149",maxWidth:"80%",display:"flex",alignItems:"center",gap:12}}>
    {progress && <div role="progressbar" aria-label={t("PDF 上传进度")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.percent} aria-valuetext={`${progress.percent}% · ${label}`} style={{width:44,height:44,flexShrink:0,position:"relative"}}>
    ...
    {!progress&&<button type="button" aria-label={t("关闭导入提示")} onClick={onDismiss}>×</button>}
  </div>;
}
