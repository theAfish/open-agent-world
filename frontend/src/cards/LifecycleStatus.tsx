...
export function LifecycleStatus({ agentId }: { agentId?: string }) {
  ...
  return <section className="lifecycle-status nodrag nopan">
    ...
    {snapshot?.runs.filter(r => !agentId || r.agent_id === agentId).slice(-30).reverse().map(run => <article key={run.run_id}>
      <strong>{run.status} · Run {run.run_id.slice(0, 8)}</strong>
      <p>Owner {run.lifecycle.owner_id ?? run.agent_id} · {run.lifecycle.holds_capacity ? "holds Agent capacity" : "capacity released"}</p>
      {run.lifecycle.awaiting && <p>Awaiting {run.lifecycle.awaiting}</p>}
      <p>Execution: {run.lifecycle.execution ?? "unknown"} · Cleanup: {run.lifecycle.cleanup ?? "none"}</p>
      ...
      {(["created", "running", "waiting"].includes(run.status) || ["pending", "failed"].includes(run.lifecycle.cleanup ?? "")) &&
        <button className="secondary-button" onClick={() => void action(() => worldApi.cancelRun(run.run_id))}>{run.lifecycle.cancellation_requested ? "Retry cancellation cleanup" : "Cancel Run"}</button>}
      {run.artifacts.map(a => <p key={a.version_id}>{a.name} · {a.state} · {a.version_id}
        {a.collection_id && <button className="secondary-button" onClick={() => useNodeSurfaceStore.getState().openWorkspace(a.collection_id!)}>Inspect artifact</button>}</p>)}
    </article>)}
    {!agentId && snapshot && <>
      {snapshot.commands.map(c => <p key={c.id}>Command {c.id} · {c.caller} · {c.state}{c.cleanup_error ? ` · ${c.cleanup_error}` : ""}</p>)}
      {snapshot.node_cleanup.map(c => <p key={c.node_id}>Node cleanup {c.node_id} · {c.last_error ?? "pending"} · {c.attempts} attempts</p>)}
      {snapshot.artifact_cleanup.map(c => <p key={c.version_id}>Artifact cleanup {c.version_id} · {c.cleanup_error ?? "pending"}</p>)}
      {snapshot.instances.filter(c => c.cleanup === "failed" || c.cleanup === "pending").map(c => <p key={c.id}>Instance cleanup {c.id} · {c.cleanup_error ?? c.cleanup}</p>)}
      {(!!snapshot.node_cleanup.length || !!snapshot.artifact_cleanup.length || snapshot.instances.some(c => c.cleanup === "failed")) &&
        <button className="secondary-button" onClick={() => void action(() => worldApi.lifecycle(true))}>Retry resource cleanup</button>}
    </>}
  </section>;
}
