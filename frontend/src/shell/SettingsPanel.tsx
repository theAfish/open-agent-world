    ...
    <div className="dialog-backdrop settings-backdrop" onMouseDown={(event) => {
      ...
      <form className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title" onSubmit={submit}>
        <header>
          ...
          <button type="button" className="icon-button" onClick={setOpen} aria-label="Close model settings">
            <X size={16} />
          </button>
        </header>
        ...
        <div className="settings-form">
          <label className="field-label">
            <span><Server size={11} /> Base URL</span>
            <input
              value={draft.baseUrl}
              ...
              spellCheck={false}
            />
            <small>Leave empty to use the provider's environment configuration.</small>
          </label>
          ...
          <label className="field-label">
            ...
            <input
              ...
              spellCheck={false}
            />
            <small>The key is kept in this browser session and runtime memory, not in world data.</small>
          </label>
          ...
          <label className="field-label">
            ...
            <textarea
              ...
              onChange={(event) => setDraft({ ...draft, models: event.target.value.split(/\r?\n/) })}
              spellCheck={false}
            />
            <small>One LiteLLM model name per line. Use provider prefixes such as <code>openai/</code> or <code>anthropic/</code>.</small>
          </label>
        </div>
